...
class EntriesEdit extends \WPForms\Pro\Forms\Fields\Base\EntriesEdit {
	...
	public function validate( $field_id, $field_submit, $field_data, $form_data ) {
		...
		if ( empty( $form_data['fields'][ $field_id ]['confirmation'] ) && ! empty( wpforms()->process->errors[ $form_id ][ $field_id ]['primary'] ) ) {
			wpforms()->process->errors[ $form_id ][ $field_id ] = wpforms()->process->errors[ $form_id ][ $field_id ]['primary'];
		}
	}
}
